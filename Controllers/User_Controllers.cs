...
namespace Web.Controllers
{
    ...
    public class User_Controllers : ControllerBase
    {
        ...
        [HttpPut]//Usercs user
        public IActionResult InsertPut(string cccd, string name, string birth, string usersName, string pass)
        {
            try
            {
                InsertPut(cccd,name,birth,usersName,pass,false);
                //user.Cccd, user.Name, user.Birthday, user.User, user.Password, Usercs.TypeOfAccount
            }
            catch
            (Exception ex)
            {
